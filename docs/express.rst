...
IZI also supports a dashboard for PayPal Express transactions, which
integrates with IZI's dashboard.
...
Finally, you need to modify izi's basket template to include the button that
links to PayPal.  This can be done by creating a new template
``templates/basket/partials/basket_content.html`` with content::

    {% extends 'izi/basket/partials/basket_content.html' %}
    {% load i18n %}
...
from izi and overriding the ``formactions`` block.  For this trick to work,
you need to ensure that you have ``IZI_MAIN_TEMPLATE_DIR`` in your
...
installation is set-up.  This is a working IZI install that uses PayPal
Express.
...
method::
    ...
    from paypal.express.views import RedirectView as IZIPaypalRedirectView


    class RedirectView(IZIPaypalRedirectView):
...
these in a 'generic' way within IZI:
